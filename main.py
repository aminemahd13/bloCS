...
try :
    while True :
        ...
        if direction=="haut": #Si on appuie sur z      
            if grille.haut(): #Si on peut tasser en haut (dans ce cas, ça tasse automatiquement)
                reset_screen()
                grille.affiche() #On affiche la grille et le score
                print(f"Score : {grille.score}")
                if not(grille.verif()): #Si plus aucun mouvement est possible on arrête
                    break
            else:
                print("Mouvement impossible !")
            
        elif direction=="gauche": #Si on appuie sur q      
            if grille.gauche(): #Si on peut tasser à gauche (dans ce cas, ça tasse automatiquement)
                reset_screen()
                grille.affiche() #On affiche la grille et le score
                print(f"Score : {grille.score}")
                if not(grille.verif()):
            ...
            if grille.bas(): #Si on peut tasser en bas (dans ce cas, ça tasse automatiquement)
                reset_screen()
                grille.affiche()
                print(f"Score : {grille.score}")
                if not(grille.verif()):
            ...
            if grille.droite(): #Si on peut tasser à droite (dans ce cas, ça tasse automatiquement)
                reset_screen()
                grille.affiche()
                print(f"Score : {grille.score}")
                if not(grille.verif()):
